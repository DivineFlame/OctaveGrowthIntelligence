...
// notes": "Cross-tenant privilege escalation via role assignment") - this
// file exists so that fix has a fast, direct test pinning it, not just
// the manual verification it originally shipped with.
...
test('userClaims shapes only the expected fields and coerces booleans', () => {
  const claims = userClaims({
    id: 'u1', tenant_id: 't1', role: 'DEPT_ADMIN', email: 'a@b.com',
...
test('userClaims defaults max_history_days to null when unset (not 0/undefined)', () => {
  const claims = userClaims({ id: 'u1', tenant_id: 't1', role: 'SUPER_ADMIN', email: 'a@b.com' });
...
test('userClaims treats max_history_days of 0 as 0, not null (nullish coalescing, not ||)', () => {
  const claims = userClaims({ id: 'u1', tenant_id: 't1', role: 'X', email: 'a@b.com', max_history_days: 0 });
...
test('hasRoleOrFlag allows a role outside the list when its per-role flag is set', () => {
  // The whole point of roleOrFlag() in server.js: a tenant admin can grant
  // a role like HR_ADMIN the can_view_integrations flag via PATCH
...
test('canGrantRole blocks a non-Super-Admin from granting the Super Admin role', () => {
  // This is the regression test for the cross-tenant privilege escalation
  // fix - IT_ADMIN and DEPT_ADMIN are tenant-scoped roles that share
  // USER_MANAGER_ROLES with SUPER_ADMIN, so both could previously create
  // or promote a user straight to SUPER_ADMIN, which reaches across every
...
test('canGrantRole allows granting any non-Super-Admin role regardless of caller', () => {
  // IT_ADMIN already has the same tenant-level permission flags as
  // SUPER_ADMIN in the roles table, so IT_ADMIN granting IT_ADMIN (or any
  // other tenant-scoped role) doesn't cross a privilege boundary the way
  // reaching into another tenant does - only the SUPER_ADMIN role itself
  // is gated.
